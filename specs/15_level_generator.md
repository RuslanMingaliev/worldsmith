...
- `ArmorAbsorption` variant added with `build_armor_absorption` builder in `level_generator`. Test fixture `tests/combat/armor_absorbs_damage.yaml` exists on disk and uses `level: armor_absorption`. The `level_generator` unit test (`test_build_armor_absorption`) covers the level construction path. **Partial:** The autopilot end-to-end assertion strategy described in `### ArmorAbsorption § Assertion strategy` (paired `player.armor: "< 100"` + `player.armor: "> 0"` with `wait: 180`) is **not yet realised** — the 2026-05-14 armor regen shipped the fixture as `wait: 60` + `player.alive: "true"` only, because `autopilot::get_field_value` was not extended to resolve `player.armor` in the same pass. The smoke-test version of the fixture passes `run_all_scenarios` but does not prove the absorption pipeline ran. Next regen must (a) add `player.armor` to the field-value resolver per `ir/contracts/autopilot.yaml § run_scenario § Field-value resolver` and (b) update the fixture to the spec-described assertion strategy.
