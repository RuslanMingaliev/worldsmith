...
Subsequent slices add: the default flip from `topdown` to `raycaster` (slice 5), and removal of the top-down code path (slice 6). The held-weapon body sprite (the gun visible at the bottom-center of the viewport, on which the muzzle flash visually anchors) was originally scoped to slice 4 alongside the HUD; it is **deferred** out of slice 4 and remains an open follow-up — the FPS HUD strip lands in this slice without it. See § Implementation Status / Deferred for the carry-over rationale.
