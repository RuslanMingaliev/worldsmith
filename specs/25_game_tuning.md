...
4. `armor -= saved`. If step 3 fired, this leaves `(armor=0, armor_type=None)`. If `saved` exactly equals the pre-hit pool, step 3 does not fire: armor becomes zero and the current tier remains. Later hits absorb nothing at zero armor because of step 1. This preserves the strict-underflow clamp described in knowledge and the module contract.
