...
| BOT_WAYPOINT_REACHED_TILES | 0.7 tile | Generation default — no knowledge backing. Distance under which the bot consumes the next BFS waypoint and steers toward the one after; inlined as a literal in `autopilot::bot_step` waypoint-consume guard. Spec/30 calls this a Coder degree of freedom (`coder_degrees_of_freedom` § "waypoint-consume distance"). 0.7 sits comfortably above the per-frame step (`MAX_SPEED * dt * 60 = 0.3 tile`) so consumption is reliable, and below 1.0 so the bot doesn't overshoot the waypoint center. (see reconcile_log#BOT_WAYPOINT_REACHED_TILES) |
