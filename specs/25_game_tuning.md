...
| Drop on death | Bullet clip (10 rounds) | Bullet clip (10 rounds) | knowledge/enemy_types.md § Death and Item Drops — basic drops "a clip", shotgun drops "a shotgun". The prototype only has one weapon (pistol), so shotgun-weapon drops are **deferred**; the shotgun trooper drops the same `PICKUP_BULLETS_AMOUNT = 10` bullet clip (`PickupKind::AmmoBullets`) the basic does. The constant was renamed from the pre-2026-05-18 single-pool `PICKUP_AMMO_AMOUNT` when the per-category split landed; the value is unchanged. Promotes to a distinct weapon-pickup when a second weapon ships (specs/25 § Deferred Combat Features). |
...
|----------|-------|--------|
| Drop on death | Bullet clip (10 rounds, `PickupKind::AmmoBullets`) | knowledge/enemy_types.md § Death and Item Drops ("The basic hitscan trooper drops a clip (bullet ammo) on death"); the shotgun trooper drops a shotgun in the reference but the prototype only ships a pistol, so the shotgun-trooper drop substitutes the same bullet-clip pickup (the basic-archetype drop) until a second weapon variant ships (specs/25 § Deferred Combat Features — Multiple weapons). knowledge/pickups.md § Drop on Kill ("basic hitscan trooper → small primary-ammo pickup (dropped, ~half clip-load)"). Knowledge says reference drops a *half*-clip via the "dropped" flag (5 rounds at our `PICKUP_BULLETS_AMOUNT = 10`); the prototype simplifies by spawning the existing full-amount `Pickup { kind: PickupKind::AmmoBullets, pos: enemy.pos, active: true }` at the trooper's death position, reusing the `level.pickups` channel that the per-frame pickup check already drains. The variant + constant names changed from the pre-2026-05-18 single-pool `Ammo` / `PICKUP_AMMO_AMOUNT` when the per-category split landed; the spawn position, the amount granted (10), and the activeness behavior are unchanged so `--autopilot` recordings remain byte-identical. The half-clip "dropped" flag is deferred (single-amount bullet pickup; promotes to a richer pickup model when more enemy types ship). See specs/60 § Enemy Ammo Drops. *(Generation default — knowledge says reference drops a shotgun for the shotgun trooper; we drop a bullet clip because the prototype has no shotgun weapon to model. Promotes when a second weapon ships.)*
...
### Player Ammo Pools (per category)

The player's ammunition is split into one independent pool per category. As of the 2026-05-18 ammo-split slice, two categories exist: **bullets** (consumed by the pistol) and **shells** (consumed by the deferred shotgun — pool exists but has no consumer in this slice). Each pool is a separate integer on `Player`; picking up one category never alters another (knowledge `combat_balance.md § Ammo Economy` — "The two categories are stored as independent integers ... picking up shells never changes the bullets count, and vice versa.").

| Constant | Value | Source |
|----------|-------|--------|
| PLAYER_BULLETS_INITIAL | 50 | knowledge/combat_balance.md § Ammo Economy + § Combat Constants Summary — "Starting ammo (bullets): 50". Direct match. Promoted from the prior scaled-down prototype value (12) when the per-category split landed; with the matching cap promotion (`PLAYER_BULLETS_MAX = 200`) the bot's `bullets == 0` ammo-opportunism gate (specs/30 § Pickup-Seeking § Ammo opportunism) does not fire in existing scenarios, so existing autopilot replays remain byte-identical. |
| PLAYER_BULLETS_MAX | 200 | knowledge/combat_balance.md § Combat Constants Summary — "Max bullets: 200". Direct match. Promoted from the prior scaled-down prototype value (30); pickup acceptance condition `player.bullets < PLAYER_BULLETS_MAX` still holds at the starting pool (50 < 200), so the per-frame pickup check still consumes a bullet pickup in existing scenarios. |
| PLAYER_SHELLS_INITIAL | 0 | knowledge/combat_balance.md § Ammo Economy + § Combat Constants Summary — "Starting ammo (shells): 0 — Shotgun unfireable until a shell pickup is found". Direct match. The shells pool starts empty because no weapon that consumes it ships in this slice (shotgun is deferred); the pool exists so a shell pickup can be received. |
| PLAYER_SHELLS_MAX | 50 | knowledge/combat_balance.md § Combat Constants Summary — "Max shells: 50". Direct match. |

The earlier single-pool constants `PLAYER_AMMO_INITIAL` (12) and `PLAYER_AMMO_MAX` (30) are retired by this slice. Their consumers — `weapon_system::fire` ammo gate, `game_loop`'s per-frame pickup-check acceptance condition, the autopilot bot's ammo-opportunism gate, and the HUD ammo pane — all switch to the per-category form (`player.bullets` for the pistol's pool, `player.shells` for the deferred shotgun's pool). The scaled-down (12 / 30) values were a generation default chosen to make a single ammo pickup feel meaningful at one-enemy-per-level density; with two categories the knowledge-direct (50 / 200, 0 / 50) values fit cleanly because the bullet pickup at the prototype's `PICKUP_BULLETS_AMOUNT = 10` is still well below `PLAYER_BULLETS_MAX`, so the "refused at cap" rule for the per-frame pickup check (specs/60 § Per-Frame Pickup Check) remains exercisable.
...
| PICKUP_BULLETS_AMOUNT | 10 | knowledge/combat_balance.md § Ammo Economy — "Small bullet pickup (clip): 10 bullets". Direct knowledge value. Renamed from the prior `PICKUP_AMMO_AMOUNT` when the per-category split landed in the 2026-05-18 slice; the value (10) is unchanged so existing bullet-pickup placements are byte-identical. |
| PICKUP_SHELLS_AMOUNT | 4 | knowledge/combat_balance.md § Ammo Economy — "Small shell pickup: 4 shells". Direct knowledge value. New in the 2026-05-18 ammo-split slice — the small shell pickup grants 4 shells. The half-clip "dropped" flag (knowledge — "half = 2 from a dropped-on-kill shell pickup") is **deferred** as for bullets (specs/60 § Deferred — "Half-clip 'dropped' flag"). |
...
| Bullets (small clip) | (15.5, 7.5) | Generation default — east of the interior horizontal wall, on a natural approach line toward the enemy at (17.5, 12.5). Rationale: lies on the path the player will most likely take; reinforces the "ammo replenish before combat" loop. Sized so a player who fired wastefully on the way still has bullets for the encounter. PickupKind variant: `AmmoBullets` (renamed from the pre-slice `Ammo` when the per-category split landed in the 2026-05-18 ammo-split slice — the variant name now spells which ammo pool it grants). The default level intentionally seeds no shell pickup so existing `--autopilot` recordings (`scavenge_run`, `complete_level`, etc.) remain byte-identical with the per-category split. The shell pickup is exercised via the dedicated `ShellPickup` demo level (specs/15) and `tests/combat/shell_pickup.yaml`. |
...
| PICKUP_AMMO_SIZE_PX | 10 | — | Generation default. Bullet-pickup (`PickupKind::AmmoBullets`) sprite size. Rationale: smaller than the health pickup (12 px) — visual hierarchy: health is the more "rare and valuable" pickup, gets the larger silhouette. The name keeps the historical `PICKUP_AMMO_*` prefix (predates the bullets/shells split); the constant is now specifically the bullet-pickup size. |
| PICKUP_AMMO_COLOR | — | #FFFF00 yellow | Generation default. Bullet-pickup (`PickupKind::AmmoBullets`) sprite color. Rationale: pure yellow is the universal "ammo / pickup" color and is reused for the HUD ammo pane (`HUD_AMMO_COLOR`) so the two visually link. Distinct from `COLOR_MUZZLE_FLASH` (`#FFFF80` pale yellow) so the static pickup does not look like a transient effect. The name keeps the historical `PICKUP_AMMO_*` prefix (predates the bullets/shells split); the constant is now specifically the bullet-pickup color. |
| PICKUP_SHELL_SIZE_PX | 10 | — | Generation default — same size as `PICKUP_AMMO_SIZE_PX` so the shell pickup reads as a sibling of the bullet pickup at a glance (same "ammo-class" silhouette tier). Knowledge `pickups.md § Ammo Pickup Tiers` does not pin per-category sprite sizes (asset-driven in the reference); the same-size choice is the simplest visual-symmetry default. New in the 2026-05-18 ammo-split slice. |
| PICKUP_SHELL_COLOR | — | #E08020 warm orange | Generation default — knowledge has no hex value for the shell pickup (sprite-asset-driven in the reference). Warm orange/red is the most common community convention for shotgun shells in retro shooters (shells are typically depicted with red plastic casings); the chosen `#E08020` is a muted orange-red that is visually distinct from `PICKUP_AMMO_COLOR` (`#FFFF00` yellow — bullet pickup), from `PICKUP_HEALTH_INNER_COLOR` (`#FF2020` red — the medical cross), from `COLOR_ENEMY` (`#FF0000` pure red — live enemies), and from `COLOR_PICKUP_TINT` (`#FFCC00` golden-yellow — the screen flash). New in the 2026-05-18 ammo-split slice. |
...
The HUD ammo pane is **weapon-aware**: it displays the count for the currently equipped weapon's ammo category (the category named in the weapon definition's `ammo_category` field — see [`ir/contracts/weapon_system.yaml § PISTOL_AMMO_CATEGORY`](../ir/contracts/weapon_system.yaml)). With pistol as the only weapon in this slice, the dispatched category is always `AmmoCategory::Bullets`, the pane reads `player.bullets`, and the icon + digit color is `HUD_AMMO_COLOR` (yellow — matching the bullet on-map pickup). Visually identical to the pre-slice HUD ammo pane since the pistol's category is bullets. When the deferred shotgun ships, the renderer's dispatch reads the equipped weapon's category and switches to `player.shells` + a future `HUD_SHELL_COLOR` automatically; no spec change is needed there beyond extending the dispatch table.

Knowledge backing: `knowledge/hud.md § Numeric Widget § Weapon-aware source rebinding (primary ammo widget only)` — "the status bar declares a dedicated 'current weapon's ammo' numeric widget whose source pointer is *redirected each frame* before the redraw step. The redirect rule reads the active weapon's ammo-category field from the weapon definition table, then writes the corresponding `&player.ammo[category]` into the widget's source pointer." Our HUD pane mirrors this: the renderer reads the equipped weapon's `AmmoCategory` and reads the matching pool from `Player`.
...
| HUD_AMMO_COLOR | — | #FFFF00 yellow | Generation default — same yellow as the on-map bullet pickup (`PICKUP_AMMO_COLOR`) so the player's eye links them. Knowledge § Color / State Encoding shows the reference uses *different fonts* (tall vs short) for primary vs secondary readouts; we substitute color since our font is monolithic. With the 2026-05-18 ammo-split slice this constant is the color used when the equipped weapon's category is `AmmoCategory::Bullets` (i.e., for the pistol). A future `HUD_SHELL_COLOR` for the shotgun's shells pane is **deferred** to the shotgun-shipping slice; until then this is the only HUD ammo color. |
...
| RAYCASTER_HUD_AMMO_COLOR | reuse `HUD_AMMO_COLOR` (`#FFFF00`) | #FFFF00 yellow | knowledge/raycaster_hud.md § Bottom-Strip Palette Reference — "Short-digit yellow (active weapons-owned): a saturated yellow roughly (220, 220, 0)". `#FFFF00` is the closest 8-bit value and reuses the existing topdown `HUD_AMMO_COLOR` (specs/25 § HUD Ammo Pane) so the on-map bullet pickup, the topdown HUD ammo pane, and the FPS HUD ammo pane all share one yellow. The reference uses two distinct font weights for primary vs secondary readouts; with our monolithic font we substitute pane position (separate strip slot) instead of a separate font (knowledge `raycaster_hud.md` § Bottom-Strip Font Treatment + § Widget Layout). The FPS HUD ammo pane is **weapon-aware** (same rule as the topdown HUD ammo pane — specs/25 § HUD Ammo Pane): it reads the equipped weapon's `AmmoCategory` and dispatches the displayed pool and color. With pistol as the only weapon the dispatch is constant (`Bullets → player.bullets → RAYCASTER_HUD_AMMO_COLOR`), so this slice's visual output is unchanged from the pre-ammo-split FPS HUD. |
...
- Ammo economy — partially implemented as of the 2026-05-18 ammo-split slice: the bullets and shells pools, the small bullet pickup, the small shell pickup, per-category caps, the per-category fire gate, and per-category HUD readout dispatch are all live (specs/25 § Pickups § Player Ammo Pools; specs/60 § Ammo (Bullets) Pickup Consumption / § Shell Pickup Consumption). Still **deferred**: the inventory-expander pickup that doubles every cap (knowledge `pickups.md § Ammo Pickup Tiers`), the half-clip "dropped" flag (specs/60 § Deferred), the bullet box / shell box large pickups (knowledge `combat_balance.md § Ammo Economy` lists 50-bullet and 20-shell large variants; not shipped), the auto-weapon-switch on zero→nonzero ammo (knowledge `pickups.md § Ammo Pickup Tiers`; meaningless with one weapon — promotes when a second weapon ships), and the energy-cell / explosive-missile categories (4-category roster — knowledge `combat_balance.md § Ammo Economy`; deferred until the weapons that consume them ship).
