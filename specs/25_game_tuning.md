...
**Implementation Status:** Deferred — the topdown HUD currently renders the health bar and digits in a single solid color (`HUD_HEALTH_BAR_COLOR = #00C000`, equivalent to the HIGH band entry below) regardless of HP. The three-band system was the original design substitute for the reference's missing palette channel; the current Coder kept the HIGH-band color but did not implement threshold-driven swaps. The `HUD_HEALTH_BAND_HIGH_THRESHOLD` / `HUD_HEALTH_BAND_LOW_THRESHOLD` / `HUD_HEALTH_COLOR_MID` / `HUD_HEALTH_COLOR_LOW` constants are not present in `renderer.rs`. Re-implementation is straightforward (one match-expression in `draw_hud`); revisit if user feedback says the single-color bar reads as less urgent at low HP. Marked deferred 2026-05-12 Reconciler pass.
...
| HUD_HEALTH_BAR_BG | #204020 dark green | Generation default — empty-bar fill. Inlined in `renderer::draw_hud` as the background rect color, drawn behind the green fill. Tinted slightly green (rather than the previously documented neutral `#303030` dark gray) so the empty portion reads as a "depleted health channel" thematically linked to the green fill, not as level chrome. Renamed from `HUD_HEALTH_BAR_BG_COLOR` to `HUD_HEALTH_BAR_BG` 2026-05-12 Reconciler pass to match the in-code identifier; value updated from `#303030` → `#204020`. (see reconcile_log#HUD_HEALTH_BAR_BG) |
| HUD_HEALTH_BAR_COLOR | #00C000 green | Inlined in `renderer::draw_hud` as the single solid color used for both the bar fill and the digits. Equivalent in hex value to the prior `HUD_HEALTH_COLOR_HIGH`; functions as the only health color since the three-band system is deferred (see § Health Bands). Captured 2026-05-12 Reconciler pass — the in-code identifier is `HUD_HEALTH_BAR_COLOR` (private const), not `HUD_HEALTH_COLOR_HIGH`. (see reconcile_log#HUD_HEALTH_BAR_COLOR) |
| HUD_HEALTH_COLOR_HIGH | #00C000 green (deferred — see § Health Bands) | Generation default — full/healthy state. Pure green is the universal "OK / safe" signal; matches the player disc color (`COLOR_PLAYER` `#00FF00`) so player and full-health bar visually agree. Not declared as a separate constant in current code — the same hex value is used as `HUD_HEALTH_BAR_COLOR` (above), which is the single color regardless of band. |
| HUD_HEALTH_COLOR_MID | #C0C000 yellow (deferred — see § Health Bands) | Generation default — middle band. Standard yellow caution; sits between green and red on the hue circle so the band transition is monotonic and intuitive. Not declared in current code. |
| HUD_HEALTH_COLOR_LOW | #C00000 red (deferred — see § Health Bands) | Generation default — critical band. Matches the enemy color (`COLOR_ENEMY` `#FF0000`) and damage tint (`COLOR_DAMAGE_TINT`) so "low health" thematically links to "the enemy is winning". Not declared in current code. |
...
|----------|-------|--------|--------|
| WEAPON_RNG_SEED | `0xDEAD_CAFE_1234_5678` | `pub const` in `player_state`; `game_loop::new` imports it and passes it to `player_state::new`, which stores it on `Player.weapon_rng` (player_state contract § Player) | Generation default — arbitrary distinctive hex value. Seeds weapon damage RNG for deterministic demo recording. RNG state lives on `Player` so `weapon_system::fire` advances it through the existing `&mut Player` borrow — no module-private `static mut`, no `unsafe` (spec/80 § Safety). The seed is passed unconditionally regardless of `--autopilot`; `_shared.yaml § main_cli § rng_seeding` permits either always-fixed or mode-switched plumbing. Value and home-module updated 2026-05-12 Reconciler pass — prior canonical home was `game_loop` private; the Coder relocated to `player_state` as a `pub const` so player_state owns its own seed and game_loop imports it. Functionally equivalent (any distinctive hex meets the determinism contract). (see reconcile_log#WEAPON_RNG_SEED) |
| ENEMY_RNG_SEED | `0xBEEF_CAFE_DEAD_1234` | `enemy_logic` (module-private const `ENEMY_RNG_SEED`, embedded in every `Enemy::rng` field at construction) | Generation default — arbitrary distinctive hex value. Seeds enemy pain-check and attack-damage RNG. The Coder dropped the per-enemy seed-injection API in this regen because every enemy initialises from the same fixed seed; the in-code identifier matches this row's canonical name. Value updated 2026-05-12 Reconciler pass — prior canonical was `0xCAFE_BABE_8765_4321`; the new Coder picked a different "distinctive hex" but the determinism property is unchanged. (see reconcile_log#ENEMY_RNG_SEED) |
...
| ~~RAYCASTER_SPRITE_SIDE_CONE_FACTOR~~ | ~~4.0~~ (not declared in current code) | knowledge/raycaster_sprites.md § World → Camera-Space Transform — "Side-cone reject". At 90° FOV the precise edge is `\|right_offset\| = forward_dist`; the reference uses ×4 slack as a coarse early-out before the per-column screen-x clip path runs. **Removed from `raycaster.rs` 2026-05-12 Coder pass** — the Coder picked the always-clip form (no early-out), and `pub const` with no in-crate caller fails `-D warnings`. The behavior is correctness-equivalent. The constant can be reintroduced when an early-out optimization materializes. (see reconcile_log#RAYCASTER_SPRITE_SIDE_CONE_FACTOR) |
...
| ~~RAYCASTER_TRACER_THICKNESS_PX~~ | ~~1~~ (not declared in current code) | Generation default — knowledge `raycaster_effects.md` § Hitscan Trace Endpoint: NO Tracer Line explicitly notes the reference does NOT render a tracer; the tracer in raycaster mode is a deliberate genre-style departure for visual parity with the topdown renderer (specs/45 § Generation Default Deviation: Tracer Line). **Removed from `raycaster.rs` 2026-05-12 Coder pass** — the tracer is drawn as a single-pixel line in `raycaster::draw` (no thickness parameter), and `pub const` with no in-crate caller fails `-D warnings`. Behavior matches the prior constant value (1 px). Reintroduce as a public constant when a thicker tracer style is implemented. (see reconcile_log#RAYCASTER_TRACER_THICKNESS_PX) |
