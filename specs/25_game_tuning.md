...
| ~~BOT_WAYPOINT_REACHED_TILES~~ | ~~0.7 tile~~ | **Superseded** — current Coder picked tile-equality (`bot.path[0] == current_tile` where `current_tile = floor(player.pos)`) instead of a distance threshold. Tile-equality is one of the unspecified shapes under `coder_degrees_of_freedom § "waypoint-consume distance"`; both meet the spec. Row retained for history; do not reintroduce as a named constant unless a future Coder reverts to a distance-threshold mechanism. (see reconcile_log#BOT_WAYPOINT_REACHED_TILES) |
