...
- `0 <= player.armor <= PLAYER_ARMOR_MAX` (= 200) — enforced by the armor pool's target-pool overwrite values (max writes 200) and by the `take_damage` clamp (`saved = min(armor, ...)` prevents negative).
