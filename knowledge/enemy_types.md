...
- **Behavior**: Every enemy has a set of states forming a finite state machine. Each state specifies a sprite frame, a duration in ticks (the reference engine runs at 35 ticks/sec), an optional action function, and the next state. The enemy transitions automatically when a state's timer expires, and can be forced into a different state by external events (being damaged, spotting a player).
- **Rules**:
  - **Idle (Spawn)**: Two frames alternating with 10-tick durations. Each frame calls the look-for-target routine to scan for players. The enemy stands in place, cycling between two frames, until it spots a target.
  - **Chase (See)**: Eight frames cycling with 4-tick durations. Each frame calls the chase routine, which handles movement toward the player, attack checks, and direction changes. A full animation cycle is 32 ticks (~0.91 seconds).
  - **Attack (Missile)**: Three frames — a 10-tick wind-up (face target), an 8-tick fire frame (deal damage), and an 8-tick cooldown. Total attack duration is 26 ticks (~0.74 seconds). After attacking, returns to chase.
  - **Pain**: Two frames of 3 ticks each (6 ticks total, ~0.17 seconds). The pain interruption is brief, making enemies recover quickly.
  - **Death**: Five frames of 5 ticks each. On frame 3, the enemy becomes non-solid (walkable). The final frame has a duration of -1, meaning it persists forever as a corpse.
  - **Gib Death (Extreme)**: Nine frames of 5 ticks each. Triggered when damage overkills by more than the enemy's spawn health. A more dramatic death animation.
  - **Resurrection**: Four frames of 5 ticks each, playing the death animation in reverse. Returns to chase state. Only possible if the enemy type has a raise state defined.
- **Constants**:
  - Tick rate: 35 per second
  - Idle frame duration: 10 ticks (~0.29 seconds per frame)
  - Chase frame duration: 4 ticks (~0.11 seconds per frame)
  - Attack total duration: 26 ticks (~0.74 seconds)
  - Pain total duration: 6 ticks (~0.17 seconds)
  - Death frame duration: 5 ticks each
- **Feel**: The relatively long idle scan (20 ticks per cycle) gives a relaxed, patrolling feel. Once alerted, the fast 4-tick chase frames make enemies feel urgent and aggressive. The very short pain duration means stun-locking weak enemies is possible with rapid fire but not trivially easy.
...
- **Rules**:
  ...
  - Visual scan: The look-for-target routine cycles through players (up to 4 in multiplayer) checking for line of sight. In the default (non-allaround) mode, it only looks in a 180-degree forward arc. Enemies farther than melee range who are behind the enemy are ignored. Line of sight uses the map's reject table for fast elimination, then traces a line checking for solid geometry.
...
- **Rules**:
  - **Reaction time**: On each chase frame, the reaction time counter decrements by 1. While reaction time is non-zero, the enemy will not attack (but will still chase). The basic trooper starts with 8 ticks of reaction time, giving the player a brief grace period after being spotted.
  - **Target threshold**: After acquiring a target (especially by being shot), the enemy tracks a "threshold" counter that decrements each chase tick. While the threshold is active, the enemy stubbornly pursues its current target even if shot by something else. The threshold is set to 100 when a new target is acquired from damage.
  ...
  - **Ranged attack check**: The enemy will attempt a ranged attack if: (a) on Nightmare difficulty or movecount is zero, (b) it has line of sight to the target, (c) it passes the missile-fire-range check. After attacking, sets a flag to prevent immediate re-attack.
...
- **Constants**:
  - Reaction time: 8 ticks for the basic trooper (~0.23 seconds)
  - Target threshold: 100 ticks (~2.86 seconds of stubborn pursuit)
  ...
  - Speed: 8 world units per movement step (basic trooper)
...
### Ranged Attack Probability Check
...
### Basic Hitscan Attack (basic hitscan trooper, low HP)

- **Behavior**: The basic hitscan trooper (low HP) fires a single hitscan (instant-hit) shot, similar to the player's pistol. The attack has random spread and variable damage.
- **Rules**:
  - Faces the target before firing
  - Uses auto-aim to find the vertical slope to the target (within the standard missile range)
  - Adds horizontal spread: a random offset of approximately +/- 5.6 degrees (using the gameplay PRNG — two calls subtracted from each other, shifted left by 20 bits in angle space)
...
- **Constants**:
  - Basic hitscan trooper (low HP) pain chance: 200/256 (~78%)
  - Shotgun trooper (3-pellet hitscan) pain chance: 170/256 (~66%)
  - Rapid-hitscan trooper pain chance: 170/256 (~66%)
  - Ranged-melee hybrid (claw + projectile) pain chance: 200/256 (~78%)
  - Melee-only beast pain chance: 180/256 (~70%)
  - Floating projectile mid-tier pain chance: 128/256 (50%)
  - Heavy melee+projectile boss pain chance: 50/256 (~20%)
  - Mid-tier melee+projectile boss pain chance: 50/256 (~20%)
  - Rocket-launcher mega-boss pain chance: 20/256 (~8%)
  - Super-chaingun mega-boss pain chance: 40/256 (~16%)
  - Kamikaze flyer pain chance: 256/256 (100%, always flinches)
- **Feel**: High pain chance on weak enemies (basic troopers, ranged-melee hybrids) means they can be effectively stun-locked with rapid fire, making them feel manageable individually. Low pain chance on bosses means they shrug off hits and keep attacking, feeling relentless. This single number dramatically changes the feel of each enemy encounter.

### Melee Range Detection

- **Behavior**: Determines if an enemy is close enough for a melee attack. The basic hitscan trooper has no melee attack, but this is used by melee-only beasts, ranged-melee hybrids, and other melee-capable enemies.
- **Rules**:
...
- **Rules**:
  ...
  - Gib/extreme death: Health drops below negative spawn health (e.g., below -20 for the basic hitscan trooper)
  - On death frame 3, the enemy loses its solid flag via the death-falling-and-deactivate routine, becoming walkable
  - Height is quartered on death (for collision purposes during the animation)
  - The basic hitscan trooper drops a clip (bullet ammo) on death
  - The shotgun trooper drops a shotgun on death
  - Death animation ticks are randomized slightly (subtract 0-3 ticks from the first frame), adding visual variety
  - Corpses can be resurrected by the area-attack boss with corpse-resurrect enemy type if they have a raise state
- **Constants**:
  - Gib threshold: -spawnhealth (e.g., -20 for basic trooper)
  - Basic hitscan trooper drop: Ammo clip
  - Shotgun trooper drop: Shotgun
  - Death tick randomization: 0-3 ticks subtracted from first frame
...
- **Rules**:
  ...
  - Floating enemies (like the floating projectile mid-tier) adjust height when blocked vertically
  - On the ground, enemy z is snapped to floor height after each move
- **Constants**:
  - Basic trooper speed: 8 (world units per move step)
...
- **One AI fits all**: Nearly every enemy in the game uses the exact same look-for-target/chase/attack framework. The dramatic difference in feel between a lowly basic trooper and a heavy melee+projectile boss comes entirely from tuning numbers (health, speed, pain chance, damage, attack type). This is an extraordinarily efficient design.

- **Pain chance is the most important balancing knob**: A 78% pain chance makes basic troopers feel manageable (they flinch often). A 20% pain chance makes heavy bosses feel imposing (they almost never flinch). The kamikaze flyer at 100% always flinches but compensates with aggressive charge attacks. This single value shapes the entire feel of each enemy encounter.

- **Hitscan vs. projectile is a fundamental enemy design axis**: The basic hitscan trooper uses hitscan (instant, cannot be dodged after firing). This makes it dangerous despite low damage because the player cannot react after the shot is fired — only prevention (killing or interrupting before the attack) works. Contrast with projectile enemies where dodging is the primary counterplay.
...
- **The "reaction time" grace period is essential**: The 8-tick delay before a newly alerted enemy can attack gives the player a critical window to react. Without it, turning a corner into enemies would be instant death. This is a subtle but vital design choice.
...
- How exactly does the homing-missile boss's tracking work? (Separate homing-missile tracking routine)
- What are the precise damage formulas for melee attacks on melee-only beasts, heavy bosses, and ranged-melee hybrids? (Each uses its own per-archetype melee/ranged attack routine)
- How does infighting work when enemies damage each other? (Target switching, species-based exceptions)
- How does the area-attack boss's resurrection mechanic select corpses and what are its constraints?
- What are the Nightmare difficulty modifiers for enemy behavior? (Faster attacks, respawning)
- How does the spawner that emits kamikaze flyers work and what limits it?
