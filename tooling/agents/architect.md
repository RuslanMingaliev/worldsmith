...
- [ ] **Enumerable feature sets pinned in IR contract.** When a spec lists an enumerable set of features implemented by a single function (e.g. `specs/30 § Available Fields` → `autopilot::get_field_value`, `specs/35 § CLI` → `main.rs::parse_args` flag set, `Archetype` variants → `enemy_logic::archetype_stats`), the corresponding `ir/contracts/<module>.yaml` MUST pin the full enumerable set explicitly in the function's contract block — not just "the function exists, see spec". Without an explicit pin the next Coder reads the spec, ships a subset, and the silent drop is invisible to Reconciler unless it manually cross-walks the resolver against the spec. The 2026-05-14 armor regen shipped `autopilot::get_field_value` without the `player.armor` arm because `ir/contracts/autopilot.yaml § run_scenario` only said "MUST recognise the fields in spec/30" without enumerating them; Reconciler retroactively added the `Field-value resolver` subsection listing each `"field" -> AssertValue` mapping. Pattern: every spec's `Available X` / `Supported X` table needs a matching contract block that names each entry as a `MUST` requirement.
