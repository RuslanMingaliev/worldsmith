...
- **Dead trait impls on shared types → spec/80 § API Surface spirit-violation.** Rust's `dead_code` lint does NOT fire on `impl <Trait> for <Type>` blocks (extension points are conservatively kept), so `cargo build -D warnings` is green even when 3-of-4 operator impls have no caller. Mechanical check: `grep -nE '^impl[[:space:]]+(std::ops|core::ops|std::cmp|std::fmt)' generated/game/src/*.rs` — for each hit, grep the workspace for at least one non-test caller of the trait operation (`+ Vec2` for `Add`, `+= Vec2` for `AddAssign`, etc.). Dead impls go under `### Drift found` with note for next Coder pass to delete. The 2026-05-12 release regen shipped dead `impl Add`, `impl Mul<f32>`, `impl AddAssign` for `Vec2`; only `impl Sub` had callers (D2).
