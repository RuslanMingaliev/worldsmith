...
- **`#[allow(dead_code)]` on any `pub` symbol → spec/80 § API Surface violation.** Coder Quality Checklist forbids dead-code masking on `pub` symbols ("no API for future use"); `cargo build` cannot warn on a masked symbol, so Reconciler must grep. Mechanical check: `grep -nE '^\s*#\[allow\(dead_code\)\]' generated/game/src/*.rs` — every hit on a line above a `pub` item is a violation, log under `### Drift found` and either (a) the Coder can make the symbol live this pass — fix-forward, or (b) the contract over-specified — drop the symbol from the contract shard in this Reconciler pass so the next regen omits it. The 2026-05-08 release regen shipped `#[allow(dead_code)]` on `pub struct Scenario` because two of its fields (`description`, `scenario`) had no non-test consumer, and the prior Reconciler did not grep for it; same failure mode as the 2026-05-07 dead-pubs cleanup (commit 17dd016). The 2026-05-14 armor-slice release regen shipped `#[allow(dead_code)] pub const PLAYER_ARMOR_MAX: u8 = 200;` while the `artifacts/coder_report.md § API Surface compromises` section read "None. All public symbols have live consumers. No `#[allow(dead_code)]` masks applied." — i.e. the Coder report directly contradicted the on-disk code. **Cross-check the grep result against the `### API Surface compromises` section of `artifacts/coder_report.md`: any masked-pub hit NOT listed there is a Coder-report fidelity gap, log it under `### Drift found` as such (separately from the spec/80 violation itself) so PostMortem can track Coder-report falsehoods independently from API-surface masking.**
- **Coder-report framing non-compliance (release-mode runs only).** `coder.md § Release regeneration mode` explicitly forbids the phrases "Unchanged", "already correct", "carried from prior session", "carry-forward", "pre-existing modules untouched", "no changes needed" in the `### Modules generated` list — every module must be described as what was emitted this run. Grep: `grep -nE 'Unchanged|already correct|carried from|carry-forward|pre-existing|no changes needed' artifacts/coder_report.md`. Any hit is non-compliance per `coder.md`; log under `### Drift found` as "Coder report framing non-compliance per coder.md § Release regeneration mode" with the offending lines. This pattern has recurred in three consecutive release-style regens (2026-05-11: 10 modules; 2026-05-12: 7 modules; 2026-05-14: 7 modules) despite explicit prompt prohibition — the Reconciler-side grep is the mechanical enforcement gate.
