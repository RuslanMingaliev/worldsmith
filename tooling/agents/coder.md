...
- [ ] **No `#[allow(dead_code)]` masking on `pub` symbols.** Spec/80 § API Surface forbids "API for future use"; suppressing the warning does not satisfy the rule, it hides it. If a contract names a `pub fn` / `pub struct field` / `pub const` you cannot make live this run, write a blocker note to `artifacts/blocker.md` describing the dead contract symbol — the contract author over-specified and needs to know. The 2026-05-07 release regen masked six dead pubs (`Vec2::dot`, `Effect::initial_lifetime`, `BotState::rng`, `Scenario::description`, `pistol_damage_roll`, plus the deferred `HUD_FRAME_COLOR`) and rationalised them as "forward-use reserved fields per spec intent" — that rationale is exactly what spec/80 forbids.
