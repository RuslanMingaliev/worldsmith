...
- [ ] **Test-count parity vs `origin/generated-snapshot` baseline.** Before submitting, run `for f in generated/game/src/*.rs; do printf '%s %s %s\n' "$(grep -c '#\[test\]' $f)" "$(git show origin/generated-snapshot:src/$(basename $f) 2>/dev/null | grep -c '#\[test\]' || echo 0)" "$(basename $f)"; done` and surface every module where POST < PRE. If any module dropped tests, either restore them verbatim from `git show origin/generated-snapshot:src/<module>.rs` or document each drop under "Skipped spec features" with the contract/spec change that made the test structurally stale. Bare "N / N pass" without a baseline comparison is a release-blocker on the Reconciler side (reconciler.md Step 0 § Test-count parity check) — and the Coder is the closest agent to the drop. The 2026-05-08 regen reported `35 / 35 pass` from baseline 64 (−29) and the 2026-05-11 release regen reported `61 / 61 pass` from baseline 77 (−16); both shipped uncaught coverage regressions because the Coder did not compare. Catching it here saves a Reconciler escalation and a follow-up Coder pass.
...
### API Surface compromises and contract simplifications
- [module.symbol]: [`#[allow(dead_code)]` applied, `#[cfg_attr(not(test), allow(dead_code))]` applied, `pub` field with no live consumer, contract field/variant shape simplified (e.g. `Kill(String)` → unit `Kill` for serde reasons), blocker note deferred, etc.] — disclose every spec/80 § API Surface workaround AND every contract-shape deviation the Coder phase took. **This is the canonical home for both classes of deviation; do NOT file them under "Compilation Fixes Applied During Generation" or "Test failures (serde / runtime)" or any other section** (the 2026-05-09 regen filed four masks under "Compilation Fixes"; the 2026-05-11 regen filed `Objective::Kill(String) → unit Kill` and `Objective::Approach(String) → unit Approach` under "Test failures (serde / runtime)" — Reconciler had to re-discover both classes mechanically as drift D2). Reconciler greps `#[allow(dead_code)]` and `#[cfg_attr(not(test), allow(dead_code))]` mechanically (reconciler.md § Step 0), but contract-shape simplifications are only catchable by reading the Coder's intent — pre-disclosing here lets Reconciler pair the deviation with your rationale and decide whether the contract is stale (update IR) or the simplification is wrong (next Coder pass restores the field). Write "None." if no compromises were taken.
