...
- [ ] **No dead trait impls on shared types.** Rust's `dead_code` lint does NOT fire on trait impls (`impl Add for Vec2`, `impl AddAssign for Vec2`, etc.) — they're extension points the compiler conservatively keeps. `cargo build -D warnings` will not catch them. Mechanical check before submitting: for every `impl <Trait> for <YourType>` in your module, grep the workspace for at least one non-test caller (`<expr> + <expr>` for `Add`, `<lhs> += <rhs>` for `AddAssign`, etc.). If none exists, delete the impl — spec/80 § API Surface "no dead pub exports" applies in spirit even where the letter doesn't catch it. The 2026-05-12 release regen shipped dead `impl Add`, `impl Mul<f32>`, `impl AddAssign` on `Vec2` (only `impl Sub` had callers) — Reconciler flagged as D2.
